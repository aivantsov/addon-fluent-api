...
@Service
public class FluentApiOperationsImpl implements FluentApiOperations {
    ...
    public void annotateType(JavaType javaType) {
        ...
        if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooFluentApi.class.getName())) == null) {
            ...
            JavaType rooRooFluentApi = new JavaType(RooFluentApi.class.getName());

            // Create Annotation metadata
            AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooFluentApi);
    ...
    public void setup() {
        ...
        projectOperations.addRepository("", new Repository("FluentApi Roo add-on repository", "FluentApi Roo add-on repository", "https://addon-fluent-api.googlecode.com/svn/repo"));
        
        List<Dependency> dependencies = new ArrayList<Dependency>();
        
        // Install the dependency on the add-on jar (
        dependencies.add(new Dependency("org.springframework.roo.addon.fluentapi", "org.springframework.roo.addon.fluentapi", "0.1.0.BUILD-SNAPSHOT", DependencyType.JAR, DependencyScope.PROVIDED));
